from flask import (
    Flask, Blueprint, current_app, request, make_response, jsonify, send_file, after_this_request
)
...
def create_file(name):
    ...
    (file_hash, file_extension, file_location) = save_file_from_request(request, name)
...
def delete_file(name):
    ...
    result = file_table.scan(
        FilterExpression=Attr('Hash').eq(hash_result['Item']['Hash']),
        )

    # Check if only 1 file with the hash value
    if result['Count'] == 1:
        file_hash = result['Items'][0]['Hash']
...
# Get file from request and save to a temporary directory
def save_file_from_request(request, name):
    ...
    return (file_hash, file_extension, file_location)
